...
struct UserNode {
	int total;
	int perfect;
	int *score;//dynamic memory alloc 
}; 
...
int main() {
	...
	scanf("%d%d%d", &n, &k, &m);
	for (i = 0;i < k;i++)//input score for each question
		scanf("%d", &s[i]);
	for (i = 1;i <= n;i++)//initialize the user list
		ranklist[i] = NULL;
	while (m--) {//for every submission record
